...
def multi_way_union_mts(mts: list, temp_dir: str, chunk_size: int) -> hl.MatrixTable:
    ...
    :param chunk_size: Number of MatrixTables to join per chunk
            (the number of individual VCFs that should be combined at a time)
    ...
    while len(staging) > 1:
        ...
        for i in range(n_jobs):
            ...
            # Multiway zip join produces an __entries annotation, which is an array where each
            # element is a struct containing the __entries annotation (array of structs) for that sample
            merged = hl.Table.multi_way_zip_join(to_merge, "__entries", "__cols")
            # Flatten __entries while taking into account different entry lengths at
            # different samples/variants (samples lacking a variant will be NA)
            merged = merged.annotate(
                __entries=hl.flatten(
                    hl.range(hl.len(merged.__entries)).map(
                        # Coalesce will return the first non-missing argument, so if the entry info
                        # is not missing, use that info, but if it is missing, create an entries
                        # struct with the correct element type for each null entry annotation (such
                        # as int32 for DP)
...
def main(args):  # noqa: D103
    ...
    # out_dir = dirname(output_ht)  # unused?
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "This script combines individual mitochondria coverage "
            "files and outputs a hail table with coverage annotations"
        )
    )
    parser.add_argument(
        "-i",
        "--input-tsv",
        help=(
            "Input file with coverage files to combine in tab-delimited"
            "format of participant_id, base_level_coverage_metrics, sample"
        ),
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--chunk-size",
        help=(
            "Chunk size to use for combining VCFs (the number of "
            "individual VCFs that should be combined at a time)"
        ),
        type=int,
        default=100,
    )
