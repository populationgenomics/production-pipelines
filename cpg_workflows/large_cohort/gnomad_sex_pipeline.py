...
def annotate_sex(
    ...
    path = os.path.join(tmp_prefix, 'gnomad_sex_pipeline_ploidy.ht')
    if can_reuse(path, overwrite):
        logger.info(f"reusing {path}")
    ...
    if compute_x_frac_variants_hom_alt:
        ...
        path = os.path.join(
            tmp_prefix, 'gnomad_sex_pipeline_ploidy_compute_x_frac_variants_hom_alt.ht'
        )
        if can_reuse(path, overwrite):
            logger.info(f'reusing {path}')
            ploidy_ht = hl.read_table(path)
    ...
    if compute_fstat:
        ...
        path = os.path.join(tmp_prefix, 'gnomad_sex_pipeline_ploidy_compute_fstat.ht')
        if can_reuse(path, overwrite):
            logger.info(f'reusing {path}')
            ploidy_ht = hl.read_table(path)
        else:
    ...
    if infer_karyotype:
        ...
        path = os.path.join(tmp_prefix, 'gnomad_sex_pipeline_ploidy_infer_karyotype.ht')
        if can_reuse(path, overwrite):
            logger.info(f'reusing {path}')
            ploidy_ht = hl.read_table(path)
        else:
