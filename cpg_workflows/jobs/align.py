...
def align(
    ...
    if can_reuse(sorted_bam_path):
        ...
        merge_j.image(image_path('samtools', '1.18'))
...
def _align_one(
    ...
    if aligner in [Aligner.BWAMEM2, Aligner.BWA]:
        if aligner == Aligner.BWAMEM2:
            tool_name = 'bwa-mem2'
            j.image(image_path('bwamem2', '2.2.1'))
            index_exts = BWAMEM2_INDEX_EXTS
        else:
            tool_name = 'bwa'
            j.image(image_path('bwa', '0.7.17'))
        ...
        j.image(image_path('dragmap', '1.3.0-tokenizer-next-fix'))
...
def extract_fastq(
    ...
    j.image(image_path('samtools', '1.18'))
