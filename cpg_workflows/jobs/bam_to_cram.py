...
def bam_to_cram(
    ...
    if add_rg:
        ...
        rg_cmd = f'samtools addreplacerg -@ {res.get_nthreads() - 1} -r "ID:{sg}" -r "SM:{sg}" -r "PL:PACBIO" -r "PU:{sg}" -w {input_bam.bam} -o {j.rg_bam["bam"]}'
        j.command(command(rg_cmd, monitor_space=True))
        input_bam = j.rg_bam
