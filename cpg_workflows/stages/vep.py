...
class Vep(CohortStage):
    ...
    def queue_jobs(self, cohort: Cohort, inputs: StageInput) -> StageOutput | None:
        ...
        # Use siteonly_vcf_part_paths if generated by JointGenotyping
        siteonly_part_pattern = inputs.as_dict(
            stage=JointGenotyping,
            target=cohort,
        ).get('siteonly_part_pattern', None)

        if siteonly_part_pattern:
            input_siteonly_vcf_part_paths = [
                to_path(siteonly_part_pattern.format(idx=idx))
                for idx in range(scatter_count)
            ]
        else:
            input_siteonly_vcf_part_paths = None
