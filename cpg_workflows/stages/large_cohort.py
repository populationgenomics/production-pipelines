...
class MakeSiteOnlyVcf(CohortStage):
    def expected_outputs(self, cohort: Cohort) -> dict[str, Path]:
        return {
            'vcf': self.tmp_prefix / 'AS_FS_testing_outDP' / 'siteonly.vcf.bgz',
            'tbi': self.tmp_prefix / 'AS_FS_testing_outDP' / 'siteonly.vcf.bgz.tbi',
        }
    ...
    def queue_jobs(self, cohort: Cohort, inputs: StageInput) -> StageOutput | None:
        ...
        j.command(
            query_command(
                ...
                str(self.tmp_prefix / 'AS_FS_testing_outDP'),
                setup_gcp=True,
            ),
        )
