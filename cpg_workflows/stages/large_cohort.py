...
class Ancestry(CohortStage):
    def expected_outputs(self, cohort: Cohort) -> dict[str, Path]:
        return dict(
            scores=get_workflow().prefix / 'ancestry_keep_relateds' / 'scores.ht',
            eigenvalues=get_workflow().prefix / 'ancestry_keep_relateds' / 'eigenvalues.ht',
            loadings=get_workflow().prefix / 'ancestry_keep_relateds' / 'loadings.ht',
            inferred_pop=get_workflow().prefix / 'ancestry_keep_relateds' / 'inferred_pop.ht',
            sample_qc_ht=get_workflow().prefix / 'ancestry_keep_relateds' / 'sample_qc_ht.ht',
        )
...
class AncestryPlots(CohortStage):
    def __init__(self, *args, **kwargs):
        ...
        self.out_prefix = get_workflow().web_prefix / 'ancestry_keep_relateds'
