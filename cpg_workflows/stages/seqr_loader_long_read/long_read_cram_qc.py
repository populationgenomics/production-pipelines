...
@stage(required_stages=[LRCramQC])
class LRSomalierPedigree(DatasetStage):
    """
    ...
    def queue_jobs(self, dataset: Dataset, inputs: StageInput) -> StageOutput | None:
        ...
        for sequencing_group in dataset.get_sequencing_groups():
            if config_retrieve(['somalier', 'exclude_high_contamination'], False):
                verify_bamid_path = inputs.as_path(stage=LRCramQC, target=sequencing_group, key='verify_bamid')
            ...
            somalier_path = inputs.as_path(stage=LRCramQC, target=sequencing_group, key='somalier')
...
@stage(required_stages=[LRCramQC, LRSomalierPedigree], analysis_type='qc', analysis_keys=['json'])
class LRCramMultiQC(DatasetStage):
    """
    ...
    def queue_jobs(self, dataset: Dataset, inputs: StageInput) -> StageOutput | None:
        ...
        paths = []
        try:
            somalier_samples = inputs.as_path(dataset, LRSomalierPedigree, key='samples')
            somalier_pairs = inputs.as_path(dataset, LRSomalierPedigree, key='pairs')
            ...
            for qc in qc_functions():
                for key, out in qc.outs.items():
                    if not out:
                        continue
                    try:
                        path = inputs.as_path(sequencing_group, LRCramQC, key)
