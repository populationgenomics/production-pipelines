...
@stage(required_stages=ExtractVcfFromCohortMt, skipped=True)
...
class AnnotateGnomadFrequenciesWithEchtvar(CohortStage):
    ...
    def queue_jobs(self, cohort: Cohort, inputs: StageInput) -> StageOutput:
        ...
        sites_vcf = get_batch().read_input(str(inputs.as_path(cohort, ExtractVcfFromCohortMt, 'sites_only')))
...
class JumpAnnotationsFromHtToFinalMt(CohortStage):
    ...
    def queue_jobs(self, cohort: Cohort, inputs: StageInput) -> StageOutput:
        ...
        vcf = str(inputs.as_path(cohort, ExtractVcfFromCohortMt, 'vcf'))
        vcf_in = get_batch().read_input_group(**{'vcf.bgz': vcf, 'vcf.bgz.tbi': f'{vcf}.tbi'})['vcf.bgz']
