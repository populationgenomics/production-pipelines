...
* take data in a VCF format
* scatter the VCF by contig
* for each contig:
  - normalise the variants, and filter to the region of interest
  - annotate with gnomAD frequencies
  - annotate with consequences using BCFtools
  - process the annotated VCFs into a HailTable
  - process the scattered VCFs into a MatrixTable, and swap the annotations over
