...
def default_config() -> PipelineConfig:
    return PipelineConfig(
        workflow=WorkflowConfig(
            ...
            check_inputs=False,
            dry_run=False,  # Set to False to test output of multiqc-file-list.txt and rename-sample-map.tsv
        ),
        images={
            'multiqc': 'test_image',
...
def setup_multiqc_test(tmp_path: Path, config: PipelineConfig | None = None):
    ...
    for i in range(4):
        sg_id = f'CPG00000{i}'
        dataset.add_sequencing_group(id=sg_id, external_id=f'SAMPLE{i}')
        paths.append(tmp_path / f'mqc_input_file_{sg_id}')
...
class TestMultiQC:
    ...
    def test_if_qc_thresholds_not_provided_then_no_report_job_created(
        self, tmp_path: Path
    ):
    ...
    def test_generate_and_copy_multiqc_reports(self, tmp_path: Path):
        ...
        assert re.search(
            fr'cp output/{report_filename}.html \${{BATCH_TMPDIR}}/.*/html', cmd
        )
        assert re.search(
            fr'cp output/{report_filename}_data/multiqc_data.json \${{BATCH_TMPDIR}}/.*/json',
            cmd,
        )
...
class TestCheckReport:
    ...
    def test_raises_error_if_no_cpg_workflows_image_provided(self, tmp_path: Path):
        config = default_config()
        # When writting TOML files, dictionaries with None values are not written,
        # hence why the image {'cpg_workflows' : None} raises an error
        config.images['cpg_workflows'] = None
        config, batch, dataset, paths = setup_multiqc_test(tmp_path, config)
