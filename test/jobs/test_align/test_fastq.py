...
pytestmark = pytest.mark.parametrize(
    'aligner',
    [Aligner.BWA, Aligner.BWAMEM2, Aligner.DRAGMAP],
)


def setup_test(config: PipelineConfig, tmp_path: Path, num_pairs: int = 1):
...
def test_creates_one_align_job_for_each_pair(n: int, aligner: Aligner, tmp_path: Path):
    config = default_config()
    config, batch, sg = setup_test(config, tmp_path, num_pairs=n)
...
def test_creates_merge_job_if_pairs_greater_than_one(
    n: int, aligner: Aligner, tmp_path: Path
):
    config = default_config()
    config, batch, sg = setup_test(config, tmp_path, num_pairs=n)
    ...
    for job in merge_jobs:
        ...
        # If there is more than one pair, the merge job should them merge all
        for i in range(n):
...
def test_sorts_output_with_samtools_if_more_than_one_pair(
    ...
    config = default_config()
    _, batch, sg = setup_test(config, tmp_path, num_pairs=n)
...
def test_using_bwa_generates_alignment_correct_command(tmp_path: Path):
    config = default_config()
    _, batch, sg = setup_test(config, tmp_path, num_pairs=1)
...
def test_using_bwamem2_generates_alignment_correct_command(tmp_path: Path):
    config = default_config()
    _, batch, sg = setup_test(config, tmp_path, num_pairs=1)
...
def test_using_dragmap_generates_alignment_correct_command(tmp_path: Path):
    config = default_config()
    _, batch, sg = setup_test(config, tmp_path, num_pairs=1)
...
def test_biobambam_uses_output_from_merge_job_when_multiple_align_jobs_are_created(
    ...
    config = default_config()
    _, batch, sg = setup_test(config, tmp_path, num_pairs=5)
