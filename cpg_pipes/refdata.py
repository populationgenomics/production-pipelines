...
class RefData:
    ...
    def __init__(self, bucket: Path):
        ...
        self.somalier_1kg_labels_tsv = (
            self.bucket / 'somalier/v0/ancestry-labels-1kg.tsv'
        )
        ...
        self.dbsnp_vcf_index = (
            self.broad_ref_bucket / 'Homo_sapiens_assembly38.dbsnp138.vcf.idx'
        )
        self.hapmap_resource_vcf = self.broad_ref_bucket / 'hapmap_3.3.hg38.vcf.gz'
        self.hapmap_resource_vcf_index = (
            self.broad_ref_bucket / 'hapmap_3.3.hg38.vcf.gz.tbi'
        )
        self.omni_resource_vcf = self.broad_ref_bucket / '1000G_omni2.5.hg38.vcf.gz'
        self.omni_resource_vcf_index = (
            self.broad_ref_bucket / '1000G_omni2.5.hg38.vcf.gz.tbi'
        )
        self.one_thousand_genomes_resource_vcf = (
            self.broad_ref_bucket / '1000G_phase1.snps.high_confidence.hg38.vcf.gz'
        )
        self.one_thousand_genomes_resource_vcf_index = (
            self.broad_ref_bucket / '1000G_phase1.snps.high_confidence.hg38.vcf.gz.tbi'
        )
        self.mills_resource_vcf = (
            self.broad_ref_bucket / 'Mills_and_1000G_gold_standard.indels.hg38.vcf.gz'
        )
        ...
        self.tel_and_cent_ht = (
            self.gnomad_ref_bucket
            / 'telomeres_and_centromeres/hg38.telomeresAndMergedCentromeres.ht'
        )
        self.lcr_intervals_ht = (
            self.gnomad_ref_bucket / 'lcr_intervals/LCRFromHengHg38.ht'
        )
        self.seg_dup_intervals_ht = (
            self.gnomad_ref_bucket / 'seg_dup_intervals/GRCh38_segdups.ht'
        )
        ...
        self.kgp_hc_ht = (
            self.gnomad_ref_bucket / 'kgp/1000G_phase1.snps.high_confidence.hg38.ht'
        )
        self.mills_ht = (
            self.gnomad_ref_bucket
