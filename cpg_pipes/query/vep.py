...
def vep_json_to_ht(vep_results_paths, out_path):
    ...
    json_schema = 'Struct{assembly_name:String,allele_string:String,ancestral:String,' \
    ...
    ht = hl.import_table(
        [p for p in vep_results_paths],
        no_header=True,
        types={'f0': schema}
    )
    ht = ht.transmute(vep=ht.f0)
    # Can't use ht.vep.start for POS because it can be modified by VEP for indels:  
    ht = ht.annotate(
