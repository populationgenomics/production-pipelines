...
for each_group in [5, 10, 25, 50, 100, 250]:
    ...
    ms_vcf = f'gs://cpg-acute-care-test/talos_benchmarking/ms_vcfs_trimmed/{each_group}/{each_group}.vcf.bgz'
    if not to_path(ms_vcf).exists():
        logging.info(f'skipping {each_group}, input file {ms_vcf} doesn\'t exist yet')
        continue
