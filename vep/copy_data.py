...
def main():
    ...
    if MAKE_VEP_CACHE_TAR:
        _vep_cache(b, refs, images)
    if MAKE_LOFTEE_TAR:
        _loftee(b, refs, images)
    if PREPARE_MOUNTABLE_BUCKET:
        _uncompress(b, refs, images)
    _test(b, refs, images)
    res = b.run()
...
def _test(b: hb.Batch, refs: RefData, images: Images):
...
def _uncompress(b: hb.Batch, refs: RefData, images: Images):
    """
    ...
    gsutil cp -r * {str(refs.vep_mount)}/
    """
    j.command(wrap_command(cmd, setup_gcp=True))
    return j


def _vep_cache(b: hb.Batch, refs: RefData, images: Images):
    """
    ...
    """
    j.command(wrap_command(cmd))
    b.write_output(j.tar, str(refs.vep_cache))
    return j


def _loftee(b: hb.Batch, refs: RefData, images: Images):
    """
    ...
    """
    j.command(wrap_command(cmd))
    b.write_output(j.tar, str(refs.vep_loftee))
